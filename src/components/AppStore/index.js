...
class AppStore extends Component {
  state = {searchInput: '', tabSelected: tabsList[0].tabId}

  filterSearchResults = event => {
    this.setState({searchInput: event.target.value})
  }

  changeTabSelected = id => {
    this.setState({tabSelected: id})
  }

  render() {
    const {searchInput, tabSelected} = this.state
    const displayAppItems = appsList.filter(
      apps =>
        apps.category === tabSelected &&
        apps.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    return (
      <div className="store-background">
        <div className="store-container">
          <h1 className="store-head">App Store</h1>
          ...
          <ul className="tab-container">
            {tabsList.map(tabItems => (
              <TabItem
                tabDetails={tabItems}
                key={tabItems.tabId}
                selectTab={this.changeTabSelected}
                activeTab={tabSelected === tabItems.tabId}
              />
            ))}
          </ul>
          <ul className="app-container">
            {displayAppItems.map(appItems => (
              <AppItem appDetails={appItems} key={appItems.id} />
            ))}
          </ul>
        </div>
      </div>
